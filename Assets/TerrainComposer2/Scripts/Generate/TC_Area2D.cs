...
namespace TerrainComposer2
{
    ...
    public class TC_Area2D : MonoBehaviour
    {
        ...
        public bool SetCurrentArea(TCUnityTerrain tcTerrain, int outputId)
        {
            ...
            if (outputId == TC.heightOutput) { intResolution.x = intResolution.y = terrain.terrainData.heightmapResolution + 2; resolution2 = new Int2(intResolution.x - 2, intResolution.y - 2); }
            else if (outputId == TC.splatOutput) { intResolution.x = intResolution.y = terrain.terrainData.alphamapResolution; resolution2 = intResolution; }
            ...
            if (outputId == TC.treeOutput || outputId == TC.objectOutput) resolutionPM = new Vector2(terrain.terrainData.size.x / (resolution2.x), terrain.terrainData.size.z / (resolution2.y));
            else resolutionPM = new Vector2(terrain.terrainData.size.x / (resolution2.x - 1), terrain.terrainData.size.z / (resolution2.y - 1));
