...
namespace TerrainComposer2
{
    ...
    {
        [NonSerialized] public List<TC_SelectItem> itemList = new List<TC_SelectItem>();
            ...
            for (int i = t.childCount - 1; i >= 0; i--)
            {
                ...
                else
                {
                    ...
                    if (outputId == TC.treeOutput)
                    {
                        ...
                        {
                            TC_Area2D.current.terrainLayer.treeSelectItems.Add(selectItem);
                            selectItem.globalListIndex = TC_Area2D.current.terrainLayer.treeSelectItems.Count - 1;
                        }
                    }
                    else if (outputId == TC.objectOutput)
                    {
                        if (selectItem.spawnObject == null) selectItem.spawnObject = new TC_SelectItem.SpawnObject();
                        if (selectItem.spawnObject.go == null) selectItem.active = false;

                        if (selectItem.active)
                        {
                            TC_Area2D.current.terrainLayer.objectSelectItems.Add(selectItem);
                            selectItem.globalListIndex = TC_Area2D.current.terrainLayer.objectSelectItems.Count - 1;
