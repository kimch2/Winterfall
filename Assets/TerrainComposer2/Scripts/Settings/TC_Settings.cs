...
namespace TerrainComposer2
{
    ...
    public class TC_Settings : MonoBehaviour
    {
        ...
        public List<TCImage> imageList = new List<TCImage>();
        ...
        {
            if (masterTerrain == null) { hasMasterTerrain = false; return; }
            if (masterTerrain.terrainData == null) { hasMasterTerrain = false; return; }
            hasMasterTerrain = true;
        }

        public TC_RawImage AddRawFile(string fullPath)
        {
            if (!TC.FileExists(fullPath)) return null;
            ...
            for (int i = 0; i < rawFiles.Count; i++)
            {
                ...
                if (rawFiles[i].fullPath == fullPath)
                {
        ...
        public int AddImage(Texture2D tex)
        {
            TCImage image;
            for (int i = 0; i < imageList.Count; i++)
            {
                image = imageList[i];
                if (image.tex == tex) return i;
            }

            imageList.Add(new TCImage(tex));
