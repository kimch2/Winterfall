...
namespace TerrainComposer2
{
    static public class TC_NodeGUI
    {
        ...
        static public void DropDownMenuInput(Rect rect, TC_Node node)
        {
            ...
            AddMenuItem(menu, instanceID, node.inputTerrain, node.outputId == TC.heightOutput ? 4 : 0);
            AddMenuItem(menu, instanceID, node.inputNoise);
            AddMenuItem(menu, instanceID, node.inputShape);
            AddMenuItem(menu, instanceID, node.inputFile);
            AddMenuItem(menu, instanceID, node.inputCurrent);
            AddMenuItem(menu, instanceID, node.inputPortal);
            
            menu.ShowAsContext();
            Event.current.Use();
        }

        static public void AddMenuItem(GenericMenu menu, string instanceID, Enum e, int startIndex = 0)
        {
            int length = Enum.GetNames(e.GetType()).Length;
        ...
        static public void LeftClickMenu(Rect rect, TC_ItemBehaviour item)
        {
            ...
            if (item.GetType() == typeof(TC_Node)) itemText = "Node";
            else if (item.GetType() == typeof(TC_SelectItem)) itemText = "Item";
        ...
        static public TC_ItemBehaviour AddItem(TC_ItemBehaviour item, bool clone)
        {
            TC_ItemBehaviour itemToDrop = null;

            if (!clone)
            {
                if (item.GetType() == typeof(TC_Node)) itemToDrop = (TC_ItemBehaviour)item.Add<TC_Node>("", true);
            ...
            TD.DropItemSameLevel(item, itemToDrop, clone, false);
