...
namespace TerrainComposer2
{
    static public class TD
    {
        ...
        static public void DragDropNode(TC_ItemBehaviour item, Rect rectStartDrag, Rect rectDrop, Vector2 pos, bool isDragable = true, bool checkForPosDrop = true)
        {
                    ...
                    if (eventCurrent.type == EventType.MouseDrag)
                    {
                        ...
                        if (DragAndDrop.objectReferences.Length == 0)
                        {
                            ...
                            if (Selection.activeGameObject == item.gameObject)
                            {
                                ...
                                DragAndDrop.objectReferences = new Object[1] { item.gameObject };
            ...
            if (rectDrop.Contains(mousePos) && eventCurrent.button == 0)
            {
                ...
                if (drop || eventCurrent.type == EventType.DragUpdated)
                {
                    ...
                    {
                        Object obj = objs[i];
                        if (obj.GetType() == typeof(Texture2D)) DropTexture(item, (Texture2D)objs[i]);
        ...
        static public void DropItemSameLevel(TC_ItemBehaviour itemReceive, TC_ItemBehaviour itemToDrop, bool clone, bool checkForPrefab = true)
        {
            ...
            if (restoreRanges)
            {
                ...
                TC_Area2D.current.terrainLayer.GetItem(selectItemReceive.outputId);
            ...
            TC.refreshOutputReferences = (itemReceive.outputId == itemToDrop.outputId) ? itemReceive.outputId : 6;
        ...
        static public void CheckDropInDifferentLevel(TC_ItemBehaviour itemReceive, TC_ItemBehaviour dropItem)
        {
            if (itemReceive.outputId != dropItem.outputId && itemReceive.outputId != TC.heightOutput)
            {
                ...
                if (layerGroup != null) { layerGroup.outputId = itemReceive.outputId; TC_Area2D.current.terrainLayer.GetItem(layerGroup.outputId); }
                else if (layer != null) { layer.outputId = itemReceive.outputId; TC_Area2D.current.terrainLayer.GetItem(layerGroup.outputId); } 
            }
        }
        ...
        static public Rect DrawNode(TC_ItemBehaviour item, Vector2 pos, Color color, Color previewColor, ref bool isCulled, float activeMulti, bool nodeFoldout, bool drawMethod, bool resultNode = false, bool drawPreview = true)
        {
            ...
            if (drawEye)
            {
                mouseButton = Button(rectEye, item.visible ? texEye : texEyeClosed, true, new Color(1, 1, 1, 0.25f), Color.white, Color.white, true, false);
                if (mouseButton == 0)
                {
            ...
            if (nodeFoldout)
            {
                ...
                if (drawPreview)
                {
                    ...
                    if (item.outputId == TC.colorOutput && selectItem != null) previewTex = item.preview.tex;
                    else
                    {
                        if (nodeGroup != null) previewTex = nodeGroup.rtColorPreview;
                        else if (node != null)
                        {
                            TC_NodeGroup parent = (TC_NodeGroup)node.parentItem;
                            if (parent.itemList.Count == 1) previewTex = ((TC_NodeGroup)node.parentItem).rtColorPreview;
                        }
                        if (previewTex == null)
                        {
                            if (item.rtDisplay != null) previewTex = item.rtDisplay;
                            else if (item.rtPreview != null) previewTex = item.rtPreview;
                            else if (item.preview.tex != null) previewTex = item.preview.tex;
                        }
                    }

                    if (previewTex == null) previewTex = Texture2D.blackTexture;

                    EditorGUI.DrawPreviewTexture(rectPreview, previewTex);
