        ...
        static public void Draw(TC_LayerGroup layerGroup, bool drawMethod, ref Vector2 pos, float activeMulti, bool isFirst, bool isLast, ref float shelfLineVerticalStartY)
        {
            ...
            if (layerGroup.foldout == 2)
            {
                for (int i = layerGroup.groupResult.itemList.Count - 1; i >= 0; --i)
                {
                    ...
                    layerGroupItem = layerGroup.groupResult.itemList[i];

                    m_isLast = (i == 0);
                    m_isFirst = (i == layerGroup.groupResult.itemList.Count - 1);

                    if (layerGroupItem.layerGroup != null)
                    {
                        Draw(layerGroupItem.layerGroup, i != 0, ref pos, activeMulti, m_isFirst, m_isLast, ref m_shelfLineVerticalStartY);
                        if (!m_isLast)
                        {
                            pos.y += layerGroupItem.layerGroup.nodeFoldout ? 0 : 32;
                    ...
                    {
                        TC_Layer layer = layerGroupItem.layer;
