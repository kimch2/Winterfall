...
namespace TerrainComposer2
{
    ...
    public class TC_TerrainAreaEditor : Editor
    {
        ...
        public void DrawTerrainAreaTiles()
        {
            ...
            int countTerrain = 0;
            for (int y = 0; y < terrainArea.tiles.y; y++)
            {
                EditorGUILayout.BeginHorizontal();
                for (int x = 0; x < terrainArea.tiles.x; x++)
                {
                    if (terrainArea.terrainSelect == countTerrain) GUI.backgroundColor = Color.green;
                    if (GUILayout.Button("", EditorStyles.miniButtonMid, GUILayout.Width(25), GUILayout.Height(25)))
                    {
                        terrainArea.terrainSelect = countTerrain;
                        terrainArea.GetAll();
                        if (eventCurrent.button == 1)
                        {
                            TCUnityTerrain tcTerrain = terrainArea.terrains[countTerrain];
        ...
        public void DrawTerrain(int index, float space)
        {
            ...
            if (terrainArea.settingsTab)
            {
                ...
                if (terrainArea.settingsEditor) currentTerrain.treeDistance = EditorGUILayout.Slider(currentTerrain.treeDistance, 0.0f, 2000);
                else currentTerrain.treeDistance = EditorGUILayout.Slider(currentTerrain.treeDistance, 0.0f, 50000);
                EditorGUILayout.EndHorizontal();
            ...
            if (terrainArea.grassTab)
            {
                ...
                for (int countGrass = 0; countGrass < currentTerrain.detailPrototypes.Count; ++countGrass)
                {
                    ...
                    {
                        EditorGUILayout.LabelField("" + countGrass + ").", GUILayout.Width(23.0f));
                        currentTerrain.detailPrototypes[countGrass].prototype = EditorGUILayout.ObjectField(currentTerrain.detailPrototypes[countGrass].prototype, typeof(GameObject), true, GUILayout.Width(175.0f)) as GameObject;
                    }
                    else
                    {
                    ...
                    EditorGUILayout.LabelField("Mesh", GUILayout.Width(40.0f));
                    currentTerrain.detailPrototypes[countGrass].usePrototypeMesh = EditorGUILayout.Toggle(currentTerrain.detailPrototypes[countGrass].usePrototypeMesh, GUILayout.Width(25.0f));
            ...
            GUILayout.Space(20);
            if (terrainArea.sizeTab || terrainArea.resolutionsTab || terrainArea.treeTab || terrainArea.grassTab)
            {
                EditorGUILayout.BeginHorizontal();
        ...
        public void Apply()
        {
            if (terrainArea.sizeTab) terrainArea.ApplySize();
            if (terrainArea.resolutionsTab) terrainArea.ApplyResolution();
            if (terrainArea.splatTab)
            {
        ...
        public void DrawReset(TC_TerrainArea terrainArea, TCUnityTerrain preterrain, float space)
        {
            ...
            {
                TC_Generate.instance.ResetObjects();
            }
        }
