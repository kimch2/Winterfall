...
public class bl_Login : MonoBehaviour {
    ...
    IEnumerator LoginProcess()
    {
        ...
        bl_LoginManager.LoadingCache.ChangeText("Requesting Login...", true);
        ...
        yield return www;
        bl_LoginManager.LoadingCache.ChangeText("Getting response from the server...", true,1.2f);
        ...
        if (mSplit[0] == "Login Done")
        {
            ...
            bl_LoginManager.LoadingCache.ChangeText("Successfully logged in.", true, 0.8f);
            AlredyLogin = true;
            //get vals
            ...
            if (GetComponent<bl_LoginManager>().DetectBan)
            {
                ...
                if (bl_BanDetect.isChecked)
                {
                    ...
                    if (!bl_BanDetect.isBanned)
                    {
                        ...
                        if (!GetComponent<bl_BanDetect>().isBanAccount(t_name))
                        {
