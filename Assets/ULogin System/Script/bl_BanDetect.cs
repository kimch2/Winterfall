...
public class bl_BanDetect : MonoBehaviour {
    ...
    void DetectIsBan()
    {
        ...
        isChecked = true;
        if (LoadWhenCheck && !isBanned) {
            SceneManager.LoadScene(GetComponent<bl_Login>().NextLevel);
            //Application.LoadLevel(this.GetComponent<bl_Login>().NextLevel);
        }
    }
    /// <summary>
    /// 
    ...
    void isBan(string n,string r,int _type)
    {
        ...
        if (_type == 0)
        {
            BanInfoText.text = "This IP has been banned due to <color=#F5A822>" + r + "</color> if you think this is a misunderstanding, contact an Admin.";
        }
        else if(_type == 1)
        {
            BanInfoText.text = "Account " + n + " has been banned due to <color=#F5A822>" + r + "</color> if you think this is a misunderstanding, contact an Admin.";
        }
